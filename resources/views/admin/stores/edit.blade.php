...
@section('title', 'Edit Cabang: ' . $store->name)
@section('breadcrumb')
    ...
    <a href="{{ route('admin.stores.show', $store->id) }}">{{ $store->name }}</a><span class="separator"><i class="fa-solid fa-chevron-right"></i></span>
...
<div class="page-header">
    <div class="page-header-row align-items-center">
        <div>
            <h1 class="page-title"><i class="fa-solid fa-pen-to-square text-primary me-2"></i>Edit Data Cabang</h1>
            <p class="page-subtitle text-muted" style="font-size:13px;">Ubah informasi, kontak, dan pengaturan operasional cabang <strong>{{ $store->name }}</strong></p>
        </div>
        <div class="page-actions d-flex gap-2">
            <a href="{{ route('admin.stores.show', $store->id) }}" class="btn btn-light px-3 py-2" style="font-weight:600;border:1px solid var(--border-light);"><i class="fa-solid fa-arrow-left me-2"></i> Batal / Kembali</a>
        </div>
    </div>
</div>
            ...
            <div class="card shadow-sm border-0 mb-4" style="border-radius:var(--radius-lg); overflow:hidden;">
                ...
                <div class="card-body p-4 bg-neutral-50">
                        ...
                        <div class="col-md-4">
                            <div class="form-group mb-0">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" name="code" class="form-control border-start-0 ps-0" required value="{{ old('code', $store->code) }}" placeholder="Contoh: BDW-01" style="font-weight:600; text-transform:uppercase;">
                                </div>
                        ...
                        <div class="col-md-8">
                            <div class="form-group mb-0">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" name="name" class="form-control border-start-0 ps-0" required value="{{ old('name', $store->name) }}" placeholder="Contoh: Roket Mini Moto Cabang Bondowoso">
                                </div>
                    ...
                    <div class="form-group mb-4">
                        ...
                        <textarea name="address" class="form-control" rows="3" required placeholder="Masukkan alamat lengkap toko beserta detail jalan dan patokan...">{{ old('address', $store->address) }}</textarea>
                        ...
                        <div class="col-md-6">
                            <div class="form-group mb-0">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" name="phone" class="form-control border-start-0 ps-0" value="{{ old('phone', $store->phone) }}" placeholder="08123456789">
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group mb-0">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" name="operational_hours" class="form-control border-start-0 ps-0" value="{{ old('operational_hours', $store->operational_hours) }}" placeholder="08:00 - 21:00 (Senin - Sabtu)">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group mb-0">
                        ...
                        <textarea name="notes" class="form-control" rows="2" placeholder="Catatan internal mengenai toko ini...">{{ old('notes', $store->notes) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="card-body p-4 text-center">
                    <div class="mb-3">
                        <div id="imagePreview" style="width:100%; height:200px; border-radius:12px; background:var(--neutral-100); border:2px dashed var(--neutral-300); display:flex; align-items:center; justify-content:center; overflow:hidden; position:relative;">
                            <img id="previewImg" src="{{ $store->photo ? asset('storage/'.$store->photo) : '' }}" style="width:100%; height:100%; object-fit:cover; display:{{ $store->photo ? 'block' : 'none' }}; position:absolute; top:0; left:0;">
                            
                            <div class="text-muted text-center" id="imagePlaceholder" style="display:{{ $store->photo ? 'none' : 'block' }};">
                    ...
                    <div class="text-start">
                        <label class="form-label fw-bold" style="font-size:13px;">Perbarui Foto</label>
                ...
                <div class="card-body p-4">
                    ...
                    <div class="p-3 rounded bg-neutral-50 border border-light mb-4">
                        ...
                        <label class="form-switch d-flex align-items-center gap-3 m-0 cursor-pointer">
                            <input type="checkbox" name="is_active" {{ $store->is_active ? 'checked' : '' }} style="width:40px; height:24px;">
                            <div>
...
function previewImage(input) {
    ...
    if (input.files && input.files[0]) {
        ...
        // If they cancel file selection, we should probably keep the old image if it exists,
        // but since we can't easily detect if an old image existed without keeping track of its URL, 
        // we'll just leave whatever is currently showing.
    }
}
</script>
@endsection
